...
from src.accounting.constants import (
    REVENUE_ACCOUNT_GROUP_ID,
    GENERAL_EXPENSE_GROUP_ID,
    CREDIT,
    DEBIT,
    INCOME_STATEMENT,
)
...
from .documentation import account_group
from .helpers import generate_gl_code
...
#         if group_text:
#             code = {"cash": [1], "revenue": [15, 16],
#             "purchase": [18]}.get(group_text)
#             if code:
...
#         """
#         No group user, only organization admin staff can create chart
#         of account. This account will be propagated to
...
#         today = datetime.now()
#         if start and end:
#             r = {"due_date__range": (today - timedelta(start),
#             today - timedelta(end))}
#         elif end is None:
...
#
#         filtered = []
#         fields = ["current", "_1_30", "_61_90", "_91_180", "_181_360",
#         "_361_infinity"]
#
...
#                 continue
#             i["number"] = (
#                 {0: "INV-{}", 1: "BILL-{}"}.get(i["document_type"])
#                 .format(i["number"])
#             )
#
#             filtered.append(i)
...
#
#         values = ["account", "account__code", "account__name", "account_
#         _balance_type"]
#         d = {
...
#         self.start_date = datetime.strptime(start, "%d-%m-%Y").date()
#         if start else None
...
#         balances = {t["account"]: t["bal"] *
#         t["account__balance_type"] for t in trans}
#
#         accounts = Account.objects.filter(
#             organization_id=self.request.user.organization_id,
...
#             l = []
#             for k, g in groupby(
#                 sorted(group, key=lambda x: x["group_code"]),
...
#                         balance_type=1,
#                         then=Coalesce(
#                             Sum(F("transactions__debit") -
#                             F("transactions__credit")), 0
#                         ),
#                     ),
#                     When(
#                         balance_type=-1,
#                         then=Coalesce(
#                             Sum(F("transactions__credit") -
#                             F("transactions__debit")), 0
#                         ),
#                     ),
#                 )
#             )
...
#         for payment to teacher, Journal voucher for
...
#         .annotate(debit=Sum("transactions__debit"),
#         credit=Sum("transactions__credit"))
#     )
